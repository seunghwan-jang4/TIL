...
# 파일 업로드 / pandas 임포트 / 모든 타입은 '*' 별로 표현.
...
if uploaded_file:
    ...
    plt.hist(df[df.columns[0]], bins=20)    #hist = 히스토그램 = 분포도를 나타내는 그림.
    st.pyplot(plt)
...
if uploaded_file:
    chart = alt.Chart(df).mark_bar().encode(
        x=df.columns[0],
        y=df.columns[1]
    )   #mark_bar() = 막대그래프, mark_line() = 꺾은 선 그래프
        #encode => 알테어에 데이터를 입력해라
    st.altair_chart(chart, use_container_width=True)
    #use_comtainer_width = 웹사이트 가로 길이랑 같아야 하니? 즉 가로 길이 맞춰줌.
    
    if uploaded_file:
        sns.set_theme(style="whitegrid")    # 테마를 'whitegrid'로 설정해 줘.
        fig, ax = plt.subplots()    
    # matplotlib에서 subplot을 가져와 줘.
    #fig는 내가 직접 쓸 그림(figure)이고,
    #ax는 x랑 y의 축(axis) 값이야. 
        sns.boxplot(data=df, ax=ax) 
    #bosplot = 상자그림 = 최대값이랑 최소값이 표시된 막대그래프
    #data = 내가 쓸 데이터 프레임
    # ax = matplotlib 에서 가져온 축 값.
